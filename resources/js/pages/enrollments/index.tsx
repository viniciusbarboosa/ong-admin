...
// Interface para os dados da inscrição
...
export default function EnrollmentIndex({ enrollments }: { enrollments: PaginatedData<Enrollment> }) {

    // Mapeamento de tradução para os status que vêm do banco
    ...
    const handleUpdateStatus = (id: number, newStatus: 'accepted' | 'rejected') => {
        ...
        if (confirm(`Tem certeza que deseja ${acao} esta inscrição?`)) {
            router.patch(route('inscricoes.status', id), {
                status: newStatus
            }, {
                preserveScroll: true,
            });
                ...
                <div className="border-sidebar-border/70 dark:border-sidebar-border overflow-hidden rounded-xl border bg-white dark:bg-neutral-900">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            <thead className="border-b border-sidebar-border/70 bg-neutral-50 dark:bg-neutral-800/50">
                                <tr>
                                    <th className="px-6 py-4 font-medium text-neutral-900 dark:text-neutral-100">Usuário / Aluno</th>
                                    <th className="px-6 py-4 font-medium text-neutral-900 dark:text-neutral-100">Curso</th>
                                    <th className="px-6 py-4 font-medium text-neutral-900 dark:text-neutral-100">Documentos (RG)</th>
                                    ...
                                    enrollments.data.map((item) => (
                                        <tr key={item.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/30">
                                            <td className="px-6 py-4">
                                                <div className="font-medium text-neutral-900 dark:text-neutral-100">{item.user.name}</div>
                                                <div className="text-xs text-neutral-500">{item.user.email}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="font-medium text-[#3043B8]">{item.course.title}</div>
                                                <div className="text-[10px] text-neutral-400">ID: #{item.id}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col gap-1">
                                                    {item.rg_front_path ? (
                                                        ...
                                                        >
                                                            <FileText size={14} /> Frente RG <ExternalLink size={10} />
                                                        </a>
                                                    ) : <span className="text-xs text-red-400">Sem imagem da frente</span>}
                                                    ...
                                                    {item.rg_back_path ? (
                                                        <a
                                                            ...
                                                            className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 transition-colors"
                                                        >
                                                            <FileText size={14} /> Verso RG <ExternalLink size={10} />
                                                        </a>
                                                    ) : <span className="text-xs text-red-400">Sem imagem do verso</span>}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-center">
                                                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase ${
                                                    item.status === 'accepted' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex justify-end gap-2">
                                                    ...
                                                    >
                                                        <Check size={22} className={item.status === 'accepted' ? '' : 'group-hover:scale-110'} />
                                                    </button>
                                                    ...
                                                    >
                                                        <X size={22} className={item.status === 'rejected' ? '' : 'group-hover:scale-110'} />
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
                    ...
                    <div className="flex items-center justify-between border-t border-sidebar-border/70 px-6 py-4 bg-neutral-50/50 dark:bg-neutral-800/20">
                        <div className="text-sm text-neutral-500">
                            Mostrando {enrollments.from || 0} até {enrollments.to || 0} de {enrollments.total} alunos
                        </div>
                        <div className="flex gap-2">
                            {enrollments.links.map((link, index) => (
                                <Link
                                    key={index}
                                    href={link.url || ''}
                                    dangerouslySetInnerHTML={{ __html: link.label.replace('Previous', 'Anterior').replace('Next', 'Próximo') }}
